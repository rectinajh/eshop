  ...
  var titles = new Array();
	...
	titles["upload_advbig"] = '切换广告图片';
	titles["upload_advmin"] = '编辑广告图片';
...
function show_adv_dialog(obj) {
	var slide_id = $(obj).attr('slide_id');
	var id = $(obj).attr("rel");
	//if(DialogManager.show(id)) return;
	...
	update_dialog(id);
	if(id=='upload_advmin'){
		$("#upload_advmin_form ul").sortable({ items: 'li' });
	    var obj2 = $(".advmin_dialog li[slide_adv_id='"+slide_id+"']");
	    var pic_name = obj2.find("input[name='adv["+slide_id+"][pic_name]']").val();
	    var pic_url = obj2.find("input[name='adv["+slide_id+"][pic_url]']").val();
	    var pic_img = obj2.find("img").attr("src");	    
	    $("#upload_advmin_form ul li").removeClass("selected").hide();
	    $("input[name='slide_pic[pic_name]']").val(pic_name);
	    $("input[name='slide_pic[pic_url]']").val(pic_url);
	    $("#upload_advmin_form .type-file-file").val('');
	    $("#upload_advmin_form .type-file-text").val('');
	    obj2.show();
		$("#upload_slide_advmin").find("input[name='slide_id']").val(slide_id);
		$("#upload_slide_advmin").find("input[name='adv_class']").val($(obj).attr('class'));
	}else{
		$("#upload_advbig_form ul").sortable({ items: 'li' });
		$("#upload_slide_advbig").find("input[name='slide_id']").val(slide_id);
		$("#upload_slide_advbig").find("input[name='adv_class']").val($(obj).attr('class'));
	}
}
...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: '/index.php?m=Admin&c=Web&a=blockUpdate',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "recommend_list":
			...
			break;
		case "brand_list":
			$("#show_brand_list").load('/index.php?m=Admin&c=Web&a=getBrandList');//查询数据
...
function get_goods_class() {//查询子分类
	...
	if (gc_id>0) {
		$.get('/index.php?m=Admin&c=Web&a=getCateList&id='+gc_id, function(data) {
			$(data).each(function(){
				var obj = $(this);
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load(GET_GOODS_URL+'?'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
	...
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			'<a href="JavaScript:show_recommend_dialog('+i+');"><i class="fa fa-shopping-cart"></i>编辑商品</a></dt>'+
    			...
    			'<li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
			...
			$("#add_recommend_list").before('<div class="ncap-form-default" select_recommend_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'</ul></dd></dl></div>');
			$("#recommend_list_form 推荐模块 dd ul").sortable({ items: 'li' });
			break;
		}
	}
}
...
function get_recommend_brand() {//查询品牌
	...
	if (brand_name!='' || brand_initial!='') {
		$("#show_brand_list").load(GET_BRAND_URL+'?'+$.param({'brand_name':brand_name,'brand_initial':brand_initial}));
	}
}
...
function add_slide_adv() {//增加图片
	for (var i = 1; i <= slide_pic_max; i++) {//防止数组下标重复
		if ($("#upload_advbig_form ul li[slide_adv_id='"+i+"']").size()==0) {//编号不存在时添加
    	    var text_input = '';
			...
			');"></span><span class="thumb size-106x106" onclick="select_slide_adv('+i+');"><img src="/public/static/images/picture.gif" /></span></div>'+text_input+'</li>';
			$("#upload_advbig_form ul").append(add_html);
			select_slide_adv(i);
			break;
		}
	}
...
function select_slide_adv(pic_id) {//选中图片
    var obj = $(".advbig_dialog li[slide_adv_id='"+pic_id+"']");
    ...
    $("#upload_advbig_form ul li").removeClass("selected");
    $("input[name='pic_id']").val(pic_id);
    $("input[name='slide_pic[pic_name]']").val(pic_name);
    $("input[name='slide_pic[pic_url]']").val(pic_url);
    $("#upload_advbig_form .type-file-file").val('');
    $("#upload_advbig_form .type-file-text").val('');
    $("#upload_slide_advbig").show();
    obj.addClass("selected");
}
function slide_adv(pic_id,pic_img) {//更新图片
    var obj = $("#upload_advbig_form ul li[slide_adv_id='"+pic_id+"']");
	if (pic_img!='') {
        ...
        var pic_img = $("#upload_advbig_form ul li").first().find("img").attr("src");
        $('#bestgoods').find("a[rel=upload_advbig]").empty().html('<img src="'+pic_img+'"/>');
    }
	//DialogManager.close("upload_advbig");
}
function update_adv(slide_id,pic_img){
	var advlen = $('.advmin_dialog li').length;
    var pic_name = $("input[name='slide_pic[pic_name]']").val();
    var pic_url = $("input[name='slide_pic[pic_url]']").val();
    
	if(advlen<slide_id){
		var adli = '<li slide_adv_id="'+slide_id+'"><div class="adv-pic">'+
        '<span class="ac-ico" onclick="del_slide_adv('+slide_id+');"></span>'+
      	'<img onclick="select_slide_adv('+slide_id+');" title="" src="'+pic_img+'"/></div>'+
        '<input name="adv['+slide_id+'][pic_id]" value="" type="hidden">'+
        '<input name="adv['+slide_id+'][pic_name]" value="'+pic_name+'" type="hidden">'+
        '<input name="adv['+slide_id+'][pic_url]" value="'+pic_url+'" type="hidden">'+
        '<input name="adv['+slide_id+'][pic_img]" value="'+pic_img+'" type="hidden"></li>';
		$('.advmin_dialog').append(adli);
	}else{
	    var obj = $("#upload_advmin_form ul li[slide_adv_id='"+slide_id+"']");
		if (pic_img!='') {
		    obj.find("img").attr("title",pic_name);
		    obj.find("img").attr("src",pic_img);
	        obj.find("input[name='adv["+slide_id+"][pic_name]']").val(pic_name);
	        obj.find("input[name='adv["+slide_id+"][pic_url]']").val(pic_url);
	        obj.find("input[name='adv["+slide_id+"][pic_img]']").val(pic_img);
	    }
	}
	$('#bestgoods').find("a[slide_id="+slide_id+"]").html('<img src="'+pic_img+'"/>');
	DialogManager.close("upload_advmin");
}
function del_slide_adv(pic_id) {//删除图片
    if ($("#upload_advbig_form ul li").size()<2) {
         return;//保留一个
    }
	$("#upload_advbig_form li[slide_adv_id='"+pic_id+"']").remove();
	var slide_id = $("input[name='slide_id']").val();
	if (pic_id==slide_id) {
    	$("input[name='slide_id']").val('');
    	$("#upload_slide_advbig").hide();
	}
}
...
function toggle_theme(){
	if(adstyle == 8){
		$('#bestgoods a').each(function(i,o){
			console.log(i)
			console.log($(o).attr('class'));	
		})
		var bestgoods = '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="0" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=7]').html()+'</a>';
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this);" slide_id="1" rel="upload_advbig" class="left-b">'+$('#bestgoods a[slide_id=0]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="2" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=1]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="3" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=2]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="4" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=3]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="5" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=4]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="6" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=5]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="7" rel="upload_advmin" class="left-a">'+$('#bestgoods a[slide_id=6]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="8" rel="upload_advmin" class="left-a"></a>'
		var addli = '<li slide_adv_id="8" style="display:none;"><div class="adv-pic"><span class="ac-ico" onclick="del_slide_adv(8);"></span><img onclick="select_slide_adv(8);" title="" src=""/></div>';
			addli += '<input name="adv[8][pic_id]" value="" type="hidden"><input name="adv[8][pic_name]" value="" type="hidden"><input name="adv[8][pic_url]" value="" type="hidden"><input name="adv[8][pic_img]" value="" type="hidden"></li>'
		$('.advmin_dialog').append(addli);
		adstyle = 9;
	}else{
		$('#bestgoods a').each(function(i,o){
			console.log(i)
		...
		var bestgoods = '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="0" rel="upload_advbig" class="left-c">'+$('#bestgoods a[slide_id=1]').html()+'</a>';
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="1" rel="upload_advmin" class="left-d">'+$('#bestgoods a[slide_id=2]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="2" rel="upload_advmin" class="left-d">'+$('#bestgoods a[slide_id=3]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="3" rel="upload_advmin" class="left-d">'+$('#bestgoods a[slide_id=4]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="4" rel="upload_advmin" class="left-e">'+$('#bestgoods a[slide_id=5]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="5" rel="upload_advmin" class="left-f">'+$('#bestgoods a[slide_id=6]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="6" rel="upload_advmin" class="left-f">'+$('#bestgoods a[slide_id=7]').html()+'</a>'
		bestgoods += '<a href="javascript:;" onclick="show_adv_dialog(this)" slide_id="7" rel="upload_advmin" class="left-f">'+$('#bestgoods a[slide_id=0]').html()+'</a>'
		$('.advmin_dialog').find('li[slide_adv_id=8]').remove();
		adstyle = 8;
	}
	$('#bestgoods').empty().append(bestgoods);
}