...
class User extends Base
{
    ...
    public function index()
    {
        ...
        $order_obj = M('order'.$select_year)->where(['user_id'=>$user['user_id'],'deleted'=>0])->order('order_id DESC')->find();
        ...
        //if($order_obj){
            //获取订单状态，订单状态显示按钮，订单商品
        //    $order=$order_obj->append(['order_status_detail','order_button','order_goods','store'])->toArray();
        //}
    ...
    */
    public function jftransfer(){
    	...
    	if(IS_POST){
    		...
    		if($rs && $rss & $rsss){
    			$this->ajaxReturn(['status'=>1,'msg'=>'转账成功!','url'=>U('Home/User/account')]);
    ...
    public function withdrawals()
    {
        ...
        if (IS_POST) {
            ...
            $count=M('withdrawals')->where("user_id=$this->user_id and create_time>$starttime")->count();
            if($count>=2){
            	$this->error('您每个月最多提现两次~');
            } */
            if ($this->user['withdraw_money'] < $distribut_need) {
                $this->error('账户提现币最少达到' . $distribut_need . '才能提现');
            }
            ...
            if ($data['money'] > $this->user['withdraw_money']) {
                $this->error("你最多可提现{$this->user['withdraw_money']}.");
            }
            if($data['money']%100 !=0){
                $this->error("你提现的提现币必须为100的倍数");
            }
...
/**
    ...
    public function jifen()
    {
    	...
    	if (IS_POST) {
    		...
    		//$distribut_need = tpCache('basic.shou3'); //提现手续费
    		//限制提现次数
    		/* $starttime=mktime(0,0,0,date("m"),1,date("Y"));
    		$count=M('withdrawals')->where("user_id=$this->user_id and create_time>$starttime")->count();
    		if($count>=2){
    		...
    		if ($data['jifen'] > $this->user['pay_points']) {
    			$this->error("你最多可提现{$this->user['pay_points']}账户积分.");
    		}
    		
    
    		$jifen = M('jifenlog')->where(array('user_id' => $this->user_id, 'status' => 0))->sum('jifen');
    		if ($this->user['pay_points'] < ($jifen + $data['jifen'])) {
    			$this->error('您有提现申请待处理，本次提现积分不足');
    		}
    	...
    	$Userlogic = new UsersLogic();
    	$result = $Userlogic->get_jifenlog($this->user_id);  //提现记录
    	$this->assign('show',$result['show']);//赋值分页输出
    	$this->assign('list',$result['result']); //下线
    	return $this->fetch();
    }
