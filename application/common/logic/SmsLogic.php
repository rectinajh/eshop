...
class SmsLogic 
{
    ...
    private function realSendSms($mobile, $smsSign, $smsParam, $templateCode,$msg)
    {
        ...
        switch($type) {
            ...
            case 1:
                ...
                break;
            case 2:
                $result = $this->sendSmsBySmsbao($mobile,$smsSign,$smsParam,$templateCode,$msg);
                break;
            default:
     ...
     */
    private function sendSmsBySmsbao($mobile,$smsSign,$smsParam,$templateCode,$msg)
    {
        $user = $this->config['sms_appkey'];
        $pass = $this->config['sms_secretKey'];
        $url = 'http://api.smsbao.com/sms?u='.$user.'&p='.md5($pass).'&m='.$mobile.'&c=【'.$smsSign.'】'.$msg;
        ...
        if ($ret == 0) {
            return array('status' => 1, 'msg' => $ret);
        } else {
            error_log(date('Y-m-d H:i:s ').var_export($ret, true)."\n", 3, __DIR__.'/sms20180427.log');
            return array('status' => -1, 'msg' => $ret);
        }
