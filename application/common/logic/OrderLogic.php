...
class OrderLogic extends Model
{
     ...
     * @return array
     */
    public function addywOrder($user_id, $address_id)
    {
        ...
        foreach ($cartList as $key => $val) {
            ...
            $data['goods_price'] = $goods['exchange_integral']; // 商品价
            $data['cost_price'] = $goods['cost_price']; // 成本价
            $data['change_user_id'] = $goods['ywuser_id']; // 物主id
            $data['total_goods_price'] = $goods['exchange_integral'] * $val['goods_num'];//每个店铺的商品价格',

            $order_id = M("changegoods")->insertGetId($data);
            ...
            // 3 扣除积分 扣除余额
            if ($data['total_goods_price'] != 0)
                M('Users')->where("user_id = $user_id")->setDec('pay_points', $data['total_goods_price']); // 用户的积分减
            // 4 清空购物车
            M('Cart')->where("user_id = $user_id and selected = 1")->delete();

            // 5 记录log 日志
            $data4['user_id'] = $user_id;
            $data4['user_money'] = -$data['total_goods_price'];
            $data4['pay_points'] = -$data['total_goods_price'];
            $data4['change_time'] = time();
            $data4['desc'] = '下单消费';
            $data4['order_sn'] = $order_sn;
            $data4['change_id'] = $order_id;
            // 如果使用了积分或者余额才记录
            ($data4['pay_points']) && M("AccountLog")->add($data4);
            ...
            if ($user['oauth'] == 'weixin') {
                ...
                $wx_content = "你刚刚下了一笔订单:{$data['order_sn']}!";
            ...
            if ($res && $res['status'] == 1) {
                $store = M('users')->where("user_id = " . $data['change_user_id'])->find();
                $sender = (!empty($store) && !empty($store['mobile'])) ? $store['mobile'] : false;
                $params = array('consignee' => $data['consignee'], 'mobile' => $data['mobile']);
    ...
    public function addOrder($user_id, $address_id, $shipping_code, $invoice_title, $coupon_id = array(), $car_price, $user_note)
    {
        ...
        foreach ($car_price['store_order_amount'] as $k => $v) {
            ...
            if ($data['order_amount'] == 0) {
                ...
                $order = M('order')->master()->where(['order_sn' => $order_sn])->find();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
                ...
                foreach ($orderGoodsArr as $key => $val) {
                    ...
                    if ($val['prom_type'] == 1 || $val['prom_type'] == 2) {
                           ...
                           /*  M('flash_sale')->where("id", $val['prom_id'])->setInc('buy_num', $val['goods_num']);
                            M('flash_sale')->where("id", $val['prom_id'])->setInc('order_num'); */
        ...
        return array('status' => 1, 'msg' => '提交订单成功', 'result' => $master_order_sn, 'order_id' => $order_id); // 返回新增的订单id
    }
     ...
     * @return array
     */
    public function h_addOrder($user_id, $address_id, $shipping_code, $invoice_title, $coupon_id = array(), $car_price, $user_note)
    {
        // 仿制灌水 1天只能下 50 单  // select * from `tp_order` where user_id = 1  and order_sn like '20151217%'
        //$order_count = M('Order')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品总数量
        //if($order_count >= 50)
        ...
        foreach ($car_price['store_order_amount'] as $k => $v) {
            ...
            $data = array(
                ...
                'shipping_price' => $car_price['store_shipping_price'][$k],//'物流价格',
                'user_money' => $car_price['store_balance'][$k], // 当前订单使用的余额数量
                'coupon_price' => ($car_price['store_coupon_price'][$k]) ? '' : 0,//'使用优惠券',
                'integral' => $car_price['pointsFee'], // 使用的积分数量
                'integral_money' => $car_price['pointsFee'],//'使用积分抵多少钱',
                'total_amount' => $car_price['payables'],// 订单总额 = 商品总价 + 物流费
                'order_amount' => $car_price['store_order_amount'][$k],//'应付款金额',
                'add_time' => time(), // 下单时间
                'order_prom_id' => ($car_price['store_order_prom_id'][$k]) ? '' : 0,//'订单优惠活动id',
                'order_prom_amount' => ($car_price['store_order_prom_amount'][$k]) ? '' : 0,//'订单优惠活动优惠了多少钱',
                'store_id' => $k,  // 店铺id
                'hcredit' => $car_price['pointsFee'],
            );
//return array('status'=>0,'msg'=>"{$data['order_prom_amount']}");
            $order_id = M("Order")->insertGetId($data);
    ...
    public function getReturnGoodsIndex($sale_t, $keywords, $user_id)
    {
        ...
        $condition['confirm_time'] = array('gt', 0);
    ...
    public function recordRefundOrder($user_id, $order_id, $user_note, $consignee, $mobile)
    {
        ...
        $order_return_num = M('return_goods')
            ->where(['order_id' => $order_id, 'user_id' => $user_id, 'status' => ['neq', 5]])
            ->count();
     ...
     * @param $set_time  自动取消时间/天 默认1天
     */
    public function abolishOrder($order, $set_time = 1)
    {
        $abolishtime = time() - ($set_time * 60 * 60 * 24);
        if ($order['add_time'] < $abolishtime) {
            $result = $this->cancel_order($order['user_id'], $order['order_id']);
            return $result;
        }
    }
}