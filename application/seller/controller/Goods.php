...
class Goods extends Base
{
    ...
    public function addEditGoods()
    {
        ...
        if(empty($goods_id)){
            ...
            //限制发布商品数量，0为不限制
            $alreadyPushNum = Db::name('goods')->where(['is_on_sale' => ['neq',2], 'store_id' => STORE_ID])->count();
            $sgGoodsLimit = Db::name('store_grade')->where(['sg_id' => $this->storeInfo['grade_id']])->getField('sg_goods_limit');
            if($alreadyPushNum > $sgGoodsLimit && $sgGoodsLimit > 0){
        ...
        $plugin_shipping = Db::name('plugin')->where(array('type' => array('eq', 'shipping')))->select();//插件物流
    ...
    public function save(){
        ...
        $goods_cat_id3 = input('post.goods_cat_id3');
        ...
        $Goods->shipping_area_ids = implode(',', $shipping_area_ids);
        ...
        $Goods->virtual_indate=$data['virtual_indate'] = !empty($virtual_indate) ? strtotime($virtual_indate) : 0;
        if ($Goods->goods_id > 0) {
            ...
            $update = $Goods->isUpdate(true)->save($data); // 写入数据到数据库
            // 更新成功后删除缩略图
            if($update !== false){
                // 修改商品后购物车的商品价格也修改一下
