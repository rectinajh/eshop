    ...
    public function return_info()
    {
    	...
    	if(IS_POST){
    		...
    		if($data['status'] == 1 || $data['status'] == -1){
    			...
    			M('order_goods'.$select_year)->where(array('order_id'=>$return_goods['order_id'],'goods_id'=>$return_goods['goods_id']))->save(array('is_send'=>2));
    		}
    ...
    public function refund_info(){
    	...
    	if(IS_POST){
    		...
    		if($data['status'] == 1){
                ...
                $order_goods_num = Db::name('order_goods')->where(['order_id'=>$return_goods['order_id']])->count();
                $order_return_goods_num = Db::name('return_goods')->where(['order_id'=>$return_goods['order_id']])->count();
               if($order_return_goods_num ==$order_goods_num){
                   //说明这个订单下的退货商品全部审核通过,则这个订单状态改为作废
                   Db::name('order')->where(['order_id'=>$return_goods['order_id']])->save(['order_status'=>5]);
                   $seller_id = session('seller_id');
                   $orderLogic->orderActionLog($order,'订单作废','订单全部商品通过退款审核，订单作废',$seller_id,1);
               };
    		}
    	...
    	$user = get_user_info($return_goods['user_id']);
    	$order_goods = M('order_goods'.$select_year)
            ->where("order_id ={$return_goods['order_id']} and goods_id = {$return_goods['goods_id']} and spec_key = '{$return_goods['spec_key']}'")
            ->find();
    	$this->assign('user',$user);
    	$this->assign('order',$order);//退货订单信息
    	$this->assign('order_goods',$order_goods);//退货订单商品
