...
//        	$btn['remove'] = '移除';
    ...
    public function getOrderStatusDetail($order)
    {
        ...
        } else // 非货到付款
        {
            ...
            if ($order['order_status'] == 0 && $order['pay_code'] != 'cod'){
                return '待确认';
            }
        ...
        if ($order['order_status'] == 2){ return '待评价'; };
        if ($order['order_status'] == 3){ return '已取消'; };
        if ($order['order_status'] == 4){ return '已完成'; };
        if ($order['order_status'] == 5){ return '已作废'; };
    }
    /**
     *	处理发货单
