...
class Service extends Base{
	...
	public function refund_info(){
		if(IS_POST){
			...
			//追回赠送优惠券
			$coupon_info = M('coupon_list')->where(array('uid'=>$return_goods['user_id'],'order_id'=>$return_goods['order_id']))->find();
			if(!empty($coupon_info)){
				if($coupon_info['status'] == 1){
					$coupon = M('coupon')->where(array('id'=>$coupon_info['cid']))->find();
					$return_goods['refund_money'] = $return_goods['refund_money']- $coupon['money']; //如果优惠券被使用,那么从退款里扣
				}else{
			...
			if($data['refund_type'] == 1 && $return_goods['refund_money']>0){
				$this->paymen_refund($return_goods);//支付原路退回
			}else{
				if($return_goods['refund_money']>0){
					accountLog($return_goods['user_id'],$return_goods['refund_money'],0,'用户申请订单退款',0,$return_goods['order_id'],$return_goods['order_sn']);//退款到用户余额
				}
			}
			//使用积分或者余额抵扣部分一一退还
			...
			M('order_goods')->where(array('rec_id'=>$return_goods['rec_id']))->save(array('is_send'=>3));
	...
	public function paymen_refund($rec_goods){
            header("Content-type: text/html; charset=utf-8");
                 exit('请联系TPshop官网客服购买高级版支持原路返回功能'); 
	}
