    ...
    public function ajaxindex(){
      ...
      //  }
		...
		foreach ($orderList as $k=>$v){
			$re=M('users')->where("user_id={$v['user_id']}")->find();
			if($re['member_name']){
				$orderList[$k]['member_name']=$re['member_name'];
			}else{
				$orderList[$k]['member_name']=$re['mobile'];
			}
		}
   ...
   {                
...

    {
    	...
    	foreach($orderList as $k=>$val){
    		...
    		$re=M('users')->where("user_id={$val['user_id']}")->find();
    		...
    		$strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$val['order_sn'].'</td>';
    		if($re['member_name']){
    			$strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$re['member_name'].'</td>';
    		}else{
    			$strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$re['mobile'].'</td>';
    		}
    ...
    public function ajaxyworder_list(){
        ...
        $nickname = ($keyType && $keyType == 'change_user_id') ? $keywords :  I('change_user_id','','trim');

        if($nickname)

        {

            $store_id_arr = M('users')->where("nickname like '%{$nickname}%'")->getField('user_id',true);

            if($store_id_arr)

            {

                $condition['user_id'] = array('in',$store_id_arr);
        ...
        $count = M('changegoods'.$select_year)->where($condition)->count();
      ...
      //  }
        ...
        $orderList = M('changegoods')->where($condition)->limit("{$Page->firstRow},{$Page->listRows}")->order('add_time desc')->select();
        foreach($orderList as $k=>$v){
            $user_name=M("users")->where("user_id",$v['change_user_id'])->getField('nickname');
            $orderList[$k]['nickname']=$user_name;
        }
        $this->assign('orderList',$orderList);
    ...
    public function ywexport_orders()

    {
        ...
        $orderList = Db::name('changegoods')->field("*,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time")->where($where)->order('id')->select();
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="60px">总积分</td>';

        $strTable .= '<td style="text-align:center;font-size:12px;" width="60px">应付积分</td>';

        $strTable .= '<td style="text-align:center;font-size:12px;" width="60px">订单状态</td>';

        $strTable .= '<td style="text-align:center;font-size:12px;" width="60px">支付状态</td>';

        $strTable .= '<td style="text-align:center;font-size:12px;" width="120px">下单日期</td>';

        $strTable .= '<td style="text-align:center;font-size:12px;" width="160px">售货人</td>';
        ...
        foreach($orderList as $k=>$val){
            $username=M("users")->where('user_id',$val['change_user_id'])->getField('nickname');
            ...
            $strTable .= '<td style="text-align:left;font-size:12px;">'.$val['total_goods_price'].'</td>';
            ...
            $strTable .= '<td style="text-align:left;font-size:12px;">'.$username.' </td>';
    ...
    public function yworder_detail($order_id){
        //订单信息
        $orderInfo=M("changegoods")->where("id",$order_id)->find();
        $orderInfo['province']=M('region')->where('id',$orderInfo['province'])->getField('name');
        $orderInfo['city']=M('region')->where('id',$orderInfo['city'])->getField('name');
        $orderInfo['district']=M('region')->where('id',$orderInfo['district'])->getField('name');
        $goodsInfo=M('goods')->where('goods_id',$orderInfo['goods_id'])->find();

        //操作记录
        $accountLog=M('account_log')->where('change_id',$order_id)->find();

        $log=M('order_action')->where('change_id',$order_id)->find();

        //收货人信息
        $userInfo=M('users')->where('user_id',$accountLog['user_id'])->find();

        $this->assign('users',$userInfo);//用户信息

        $this->assign('order',$orderInfo);

        $this->assign('action_log',$accountLog);
