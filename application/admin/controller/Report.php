...
class Report extends Base{
	...
	public function index(){
		...
		$sql = "SELECT COUNT(*) as tnum,sum(order_amount) as amount, FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap from  __PREFIX__order{$this->select_year} ";
	...
	public function userTop(){		 		
		...
		$sql = "select count(a.order_id) as order_num,sum(a.order_amount) as amount,a.user_id,b.mobile,b.email,b.nickname from __PREFIX__order{$this->select_year} as a left join __PREFIX__users as b ";
		...
		$this->assign('list',$res);
//		if(empty($where)){
//			$count = M('order'.$this->select_year)->where("add_time>$this->begin and add_time<$this->end and pay_status=1")->group('user_id')->count();
...
//		}
	...
	public function saleList(){
		...
		$sql2 = "select count(*) as tnum from __PREFIX__order_goods{$this->select_year} as a left join __PREFIX__order{$this->select_year} as b on a.order_id=b.order_id ";
		...
		$sql = "select a.*,b.order_sn,b.shipping_name,b.pay_name,b.add_time from __PREFIX__order_goods{$this->select_year} as a left join __PREFIX__order{$this->select_year} as b on a.order_id=b.order_id ";
		$sql .= " left join __PREFIX__goods as g on a.goods_id = g.goods_id $where ";
		$sql .= "  order by add_time limit {$Page->firstRow},{$Page->listRows}";
		$res = DB::query($sql);
		$this->assign('list',$res);
