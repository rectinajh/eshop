<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:42:"./application/admin/view2/user/detail.html";i:1532661069;s:44:"./application/admin/view2/public/layout.html";i:1532661069;}*/ ?>
<!doctype html>
<html>
<head>
...
<body style="background-color: #FFF; overflow: auto;">
        ...
        <form class="form-horizontal" id="user_form" method="post">
            ...
            <div class="ncap-form-default">
                <dl class="row">
                    ...
                    <dd class="opt">
                        <input class="input-txt valid" name="nickname" value="<?php echo $user['nickname']; ?>" readonly="readonly" type="text">
                        <p class="notic">会员昵称不可修改。</p>
                    </dd>
                </dl>
                <dl class="row">
                    <dt class="tit">
                        <label class="qq">内测会员</label>
                    </dt>
                    <dd class="opt">
                        <input name="is_test" value="1" type="radio" <?php echo !empty($user['is_test']) && $user['is_test']==1?'checked' : ''; ?>>是
                        <input name="is_test" value="0" type="radio" <?php echo !empty($user['is_test']) && $user['is_test']==1?'' : 'checked'; ?>>否
                        <p class="notic">交易关闭后仍然允许参与交易测试</p>
                        <span class="err"></span>
                    </dd>
                </dl>
                <dl class="row">
                    <dt class="tit">
                        <label class="qq">推荐人手机号</label>
                    </dt>
                    <dd class="opt">
                        <input name="tuimobile" type="text" value="<?php echo $user['parent']['mobile']; ?>" readonly>
                        <span class="red">请务必保证推荐关系一定不会形成闭环，否则将会导致系统崩溃</span>
                        <div style="margin-top: 20px;">
                            <label><input id="force_modify_parent" name="force_modify_parent" type="checkbox" value="1">我已知晓风险并愿意自行承担一切风险</label>
                        </div>
                        <span class="err"></span>
                    </dd>
                </dl>
                ...
                <dl class="row">
                    <dt class="tit">
                        <label class="qq">会员等级</label>
                    </dt>
                    <dd class="opt">
                        <select name="level" id="level">
                            <option value="0">请选择</option>
                            <?php if(is_array($levels) || $levels instanceof \think\Collection || $levels instanceof \think\Paginator): if( count($levels)==0 ) : echo "" ;else: foreach($levels as $i=>$level): ?>
                                <option value="<?php echo $i; ?>" <?php if($user['level'] == $i): ?> selected<?php endif; ?> ><?php echo $level; ?></option>
                ...
                <dl class="row">
                    <dt class="tit">
                        <label class="qq">会员角色</label>
                    </dt>
                    <dd class="opt">
                        <select name="level" id="level">
                            <option value="0">请选择</option>
                            <?php if(is_array($roles) || $roles instanceof \think\Collection || $roles instanceof \think\Paginator): if( count($roles)==0 ) : echo "" ;else: foreach($roles as $i=>$role): ?>
                                <option value="<?php echo $i; ?>" <?php if($user['role'] == $i): ?> selected<?php endif; ?> ><?php echo $level; ?></option>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                        <span class="err"></span>
                    </dd>
                ...
                <dl class="row">
                    ...
                    <dd class="opt">
                        <strong class="red"><?php echo $user['pay_points']; ?></strong>&nbsp;积分 </dd>
                </dl>
                <dl class="row">
                    <dt class="tit">
                        <label>账户余额</label>
                    ...
                    <dd class="opt">
                        <strong class="red"><?php echo $user['user_money']; ?></strong>&nbsp;元 </dd>
                </dl>
                <dl class="row">
                    <dt class="tit">
                        <label for="email">
                ...
                <dl class="row">
                    ...
                    <dd class="opt">
                        <div class="onoff">
                            <label for="is_lock1" class="cb-enable <?php if($user[ 'is_lock'] == 1): ?>selected<?php endif; ?>">
                                <span>开启</span>
                            </label>
                            <label for="is_lock2" class="cb-disable <?php if($user[ 'is_lock'] == 0): ?>selected<?php endif; ?>">
                                <span>关闭</span>
                            </label>
            ...
            -->
                <div class="bot">
                    <a href="JavaScript:void(0);" onclick="checkUserUpdate();" class="ncap-btn-big ncap-btn-green">确认提交</a>
                </div>
            </div>
        </form>
    </div>
    <script type="text/javascript">
        $(document).ready(function() {
            var oldValue = $('input[name=tuimobile]').val();
            $('input[name=tuimobile]').prop('defaultValue', oldValue);
        });
        $('#force_modify_parent').click(function() {
            var force_checkbox = $(this), parent_mobile = $('input[name=tuimobile]');
            if (force_checkbox.prop('checked')) {
                layer.confirm('错误的推荐级关系将会导致系统崩溃，真的确定要继续吗？', {title:'警告', icon:0}, function(index) {
                    force_checkbox.prop('checked', 'checked');
                    parent_mobile.removeAttr('readonly');
                    layer.close(index);
                }, function() {
                    force_checkbox.removeProp('checked');
                    parent_mobile.attr('readonly', 'readonly');
                });
            } else {
                parent_mobile.attr('readonly', 'readonly');
                parent_mobile.val(parent_mobile.prop('defaultValue'));
            }
        });
        function checkUserUpdate() {
            ...
            $.ajax({
                url: "<?php echo U('Admin/user/detail'); ?>",
                type: 'post',
