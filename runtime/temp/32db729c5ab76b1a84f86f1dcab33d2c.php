								...
								<div class="brand-list">
									<div class="clearfix p">
										<a href="<?php echo U('Home/Activity/group_list'); ?>" >
											<span <?php if(\think\Request::instance()->param('cat_id') == null): ?>class="red"<?php endif; ?>>全部</span>
										</a>
										<?php if(is_array($cat_list) || $cat_list instanceof \think\Collection || $cat_list instanceof \think\Paginator): $i = 0; $__LIST__ = $cat_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                            <a href="<?php echo U('Home/Activity/group_list',array('cat_id'=>$vo['id'])); ?>">
                                                <span <?php if(\think\Request::instance()->param('cat_id') == $vo['id']): ?>class="red"<?php endif; ?>><?php echo $vo['name']; ?></span>
                                            </a>
										<?php endforeach; endif; else: echo "" ;endif; ?>
									</div>
								</div>
			...
			<div class="w1224">
				<div class="stsho pre-sts">
					<div class="sx_topb presellall group-act">
						<div class="f-sort fl">
							<ul>
								<li onClick="location.href='<?php echo U('Home/Activity/group_list'); ?>'" <?php if(\think\Request::instance()->param('order') == ''): ?>class="red"<?php endif; ?>><a href="<?php echo U('Home/Activity/group_list'); ?>">默认</a></li>
								<li onClick="location.href='<?php echo U('Home/Activity/group_list',array('order'=>1)); ?>'" <?php if(\think\Request::instance()->param('order') == 1): ?>class="red"<?php endif; ?>><a href="<?php echo U('Home/Activity/group_list',array('order'=>1)); ?>">最新</a></li>
								<li onClick="location.href='<?php echo U('Home/Activity/group_list',array('order'=>2)); ?>'" <?php if(\think\Request::instance()->param('order') == 2): ?>class="red"<?php endif; ?>><a href="<?php echo U('Home/Activity/group_list',array('order'=>2)); ?>">推荐</a></li>
							</ul>
							<div class="actionname">
								<span>活动名称：</span>
								<input class="text-act" type="text" id="title" value="<?php echo \think\Request::instance()->param('title'); ?>" />
						...
						<div class="f-total fr">
							...
							<div class="all-fy">
                                <a <?php if($nowPage > 1): ?>href="<?php echo U('Home/Activity/group_list',array_merge(I('get'),array('p'=>$nowPage-1))); ?>" <?php endif; ?>>&lt;</a>
                                <p class="fy-y"><span class="z-cur"><?php echo $nowPage; ?></span>/<span><?php echo $totalPages; ?></span></p>
                                <a <?php if(($nowPage+1) < $totalPages): ?>href="<?php echo U('Home/Activity/group_list',array_merge(I('get'),array('p'=>$nowPage+1))); ?>"<?php endif; ?>>&gt;</a>
                            </div>
						</div>
					</div>
					<div class="shop-list-splb pre-set-suma groupcy p">
						<ul>
							<?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?>
								...
								<?php else: if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?>
									<li>
										<div class="s_xsall">
											<div class="xs_img">
												<a target="_blank" href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$group[goods_id],'item_id'=>$group[specGoodsPrice][item_id])); ?>">
													<img class="lazy-list" data-original="<?php echo goods_thum_images($group['goods_id'],262,262); ?>"/>
												</a>
											</div>
											<div class="shop_name2">
												<a target="_blank" href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$group[goods_id],'item_id'=>$group[specGoodsPrice][item_id])); ?>">
													<?php echo getSubstr($group[title],0,30); ?>
												</a>
											</div>
											<div class="price-tag">
												<span class="now"><em class="li_xfo">5</em><em>分</em></span>
												<span class="pre-per"><em><?php echo $group[virtual_num] + $group[buy_num]; ?></em>人已参加</span>

												<div class="gb_nastr p">
													<div class="squetrian">
														<div class="arrow-right">
															<em><?php echo $group[rebate]; ?></em>&nbsp;<span>折</span>
														</div>
													</div>
													<div class="old-new-prices">
														<p class="old"><span>￥</span><span><?php echo $group['goods_price']; ?></span></p>

														<p class="new"><span>￥</span><span><?php echo $group['price']; ?></span></p>
													</div>
												</div>
											</div>
										</div>
										<div class="attendgorb-ocl">
											<a target="_blank" href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$group[goods_id],'item_id'=>$group[specGoodsPrice][item_id])); ?>" style="color: white">
												<span>立即团购<i>></i></span>
											</a>
										</div>
									</li>
