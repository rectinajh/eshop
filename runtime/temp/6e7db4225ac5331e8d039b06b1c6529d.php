<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:47:"./application/admin/view2/uploadify/upload.html";i:1532661068;}*/ ?>
<html lang="zh-cn">
...
<title>文件管理 - www.ohbbs.cn 欧皇源码论坛 </title>
...
$(function(){
	...
	$(".statusBar .saveBtn").click(function(){
		...
		if(num > 1){			//没定义回调  多图
				...
				$(window.parent.document).find("#{<1in></1in>fo.input}").append(fileurl_tmp);

		}else{     //单图

				$(window.parent.document).find("#<?php echo $info['input']; ?>").val($("input[name^='fileurl_tmp']").val());
				$(window.parent.document).find("#<?php echo $info['input']; ?>"+"s").attr('src',($("input[name^='fileurl_tmp']").val()));
